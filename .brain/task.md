...
- [ ] **LoRA Skill Modules** — LoRA-based skill cards that auto-download from CivitAI. Each skill = pre-tuned `activated_loras` + `loras_multipliers` + generation defaults (from user-provided JSON archive). **Simple/Advanced toggle is non-destructive** — toggling back to Simple hides controls but preserves custom params. **"↩ Reset to Defaults" button** in Advanced panel re-applies skill's curated `lockedParams`. Only re-selecting a skill from the dropdown resets to defaults. Multi-LoRA stacking in Advanced mode.
